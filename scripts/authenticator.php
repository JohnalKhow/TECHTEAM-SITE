...
$sqlConnect2 = mysqli_connect(HOST, USER, PASS);
if (!$sqlConnect or !$sqlConnect2) {
	die();
}
$selectDB = mysqli_select_db($sqlConnect, DB);
$selectDB2 = mysqli_select_db($sqlConnect2, DB2);
if (!$selectDB or !$selectDB2) {
	die(mysqli_error("Can't find the database!"));
}
//INIT
$countries = array(
	"AF" => "Afghanistan",
...
if (!empty($_SESSION['id']) and ($_SESSION['priveleges'] == 1)) {
	$job_data = mysqli_query($sqlConnect2, "SELECT * FROM orders WHERE id='$_SESSION[id]'");
...
if (isset($_POST["accept-btn"])) {
	...
	if (!empty($_REQUEST["order-id"])) {
		...
		$history = mysqli_query($sqlConnect2, "SELECT * FROM history WHERE orderid='$orderid' ");
		while ($SR = mysqli_fetch_array($history)) {
			$project = $SR["project"];
		}
		$data = mysqli_query($sqlConnect2, "SELECT * FROM `orders` WHERE project='$project' or project2='$project'");
		...
		if (!empty($id)) {
			if ($project1 == $project) {
				mysqli_query($sqlConnect2, "UPDATE `orders` SET handler='$_SESSION[email]' ,status1='2' WHERE id='$id' ");
				mysqli_query($sqlConnect2, "UPDATE `history` SET handler='$_SESSION[email]', status1='Assigned to Handler' WHERE orderid='$orderid'");
			} elseif ($project2 == $project) {
				mysqli_query($sqlConnect2, "UPDATE `orders` SET handler2='$_SESSION[email]', status2='2' WHERE id='$id' ");
				mysqli_query($sqlConnect2, "UPDATE `history` SET handler='$_SESSION[email]', status1='Assigned to Handler' WHERE orderid='$orderid'");
			}
		}
	}
...
if (isset($_POST["update-btn"])) {
	...
	if (empty($_REQUEST["order-id"]) or empty($_REQUEST["statusid"]) or $_REQUEST["statusid"] <= 2 or $_REQUEST["statusid"] > 5)
		...
		if (!empty($id)) {
			if ($project1 == $project) {
				mysqli_query($sqlConnect2, "UPDATE `orders` SET status1='$statusid', repository='$repositoryid' WHERE id='$id'");
				mysqli_query($sqlConnect2, "UPDATE `history` SET  status1='$statustxt', repository='$repositoryid' WHERE orderid='$orderid'");
			} elseif ($project2 == $project) {
				mysqli_query($sqlConnect2, "UPDATE `orders` SET status2='$statusid', repository2='$repositoryid' WHERE id='$id' ");
				mysqli_query($sqlConnect2, "UPDATE `history` SET  status1='$statustxt', repository='$repositoryid' WHERE orderid='$orderid'");
			}
		}
	}
...
if (isset($_POST["purchase"])) {
	...
	if (count($error_buy) == 0 and $_SESSION["proj_limit"] > 0) {
		...
		if (!$_SESSION['project']) {
			...
			mysqli_query($sqlConnect2, $update);
			...
			mysqli_query($sqlConnect2, $insert_history);
		...
		if (password_verify($password, $passwordAuth)) {
			...
			if ($_SESSION['priveleges'] == 0) {
				...
				if ($logcount == 0) {
					...
					mysqli_query($sqlConnect2, $register);
...
if (isset($_POST["cancel-1"])) {
	...
	mysqli_query($sqlConnect2, $update_hist);
	$_SESSION["project"] = "";
	$_SESSION["status"] = "";
	$_SESSION["handler"] = "";
	$_SESSION["remarks"] = "";
